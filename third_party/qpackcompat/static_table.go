...
var encoderMap = buildEncoderMap()

func buildEncoderMap() map[string]indexAndValues {
	result := make(map[string]indexAndValues)
	for index, field := range staticTableEntries {
		entry, exists := result[field.Name]
		if !exists {
			entry.idx = uint8(index)
		}
		if field.Value != "" {
			if entry.values == nil {
				entry.values = make(map[string]uint8)
			}
			entry.values[field.Value] = uint8(index)
		}
