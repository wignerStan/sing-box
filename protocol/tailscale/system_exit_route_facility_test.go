...
func TestSystemExitRouteReadinessFailsWhenInactive(t *testing.T) {
	...
	facility.Start()
	t.Cleanup(facility.Stop)
	if err := facility.RequestAndWait(context.Background()); !errors.Is(err, net.ErrClosed) {
		t.Fatalf("inactive readiness error = %v, want %v", err, net.ErrClosed)
	}
	if _, _, err := facility.updateResult(); !errors.Is(err, net.ErrClosed) {
		t.Fatalf("inactive update error = %v, want %v", err, net.ErrClosed)
	}
}
