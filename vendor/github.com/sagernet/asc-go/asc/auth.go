...
func (g *standardJWTGenerator) IsValid() bool {
	...
	options := []jwt.ParserOption{jwt.WithAudience("appstoreconnect-v1")}
	if g.issuerID != "" {
		options = append(options, jwt.WithIssuer(g.issuerID))
	}

	parsed, err := jwt.Parse(g.token, jwt.KnownKeyfunc(jwt.SigningMethodES256, g.privateKey), options...)
	if err != nil {
		return false
	}
